...
namespace SistemaLoca.BusinnesLogic.Teste
{
    ...
    public class FilmeTesteRepository
    {
        ...
        UnitOfWorkMapeamento uowm = new UnitOfWorkMapeamento();
        ...
        public void InserirFilmeTeste()
        {
            ...
            //1 passo dbContext.Filmes.Add(filme);
            uowm.FilmeRepository.Insert(filme);
            
            //1 passo dbContext.SaveChanges();
            uowm.Save();

            //1 passo filme = dbContext.Filmes.Find(1);
            filme = uowm.FilmeRepository.GetByID(1);
        ...
        {
            //1 passo Filme filme = dbContext.Filmes.Where(f => f.Titulo == "Matrix").First<Filme>();
            Filme filme = uowm.FilmeRepository. ????
        ...
        {
            // 1 passo Filme filme = dbContext.Filmes.Find(1);
            Filme filme = uowm.FilmeRepository.GetByID(1);
            Assert.AreEqual(filme.Titulo, "Matrix");
        }
